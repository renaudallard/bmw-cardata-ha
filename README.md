...
- The windows and the sunroof also get a binary sensor with the `window` device class, next to the string sensor that reports the BMW enum. They are disabled by default: enable the ones you want from the device page to use them in a `window.opened` trigger.
